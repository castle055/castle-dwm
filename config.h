...
std::vector<const char*> config::fonts  = { "Fira Code Retina:style=Bold:size=9:antialias=true:autohint:true" };
...
std::vector<std::vector<const char*>> config::colors      = {
	/*               fg         bg         border   */
	{ config::normfgcolor.c_str(),   config::normbgcolor.c_str(), config::normbordercolor.c_str()},
	{ config::selfgcolor.c_str(),   config::selbgcolor.c_str(),    config::selbordercolor.c_str()}
};
...
/* tagging */
const std::vector<std::string> config::TAGS = {"\uf269", "\uf121", "3", "4", "5", "6", "7", "8", "\uf274" };
...
std::vector<Rule> config::rules = {
    ...
    { "jetbrains-clion",NULL,  NULL,      1 << 4,       0,            1},
