  ...
  std::unordered_map<std::string, std::vector<const char*>> config::cmds = {
    {
      "dmenucmd",
      {
        "dmenu_run", "-m", "0", "-fn", config::dmenufont.c_str(), "-nb", config::selbgcolor.c_str(), "-nf",
        config::normfgcolor.c_str(), "-sb", config::normfgcolor.c_str(), "-sf", config::selbgcolor.c_str(), nullptr
      }
    },
    {"termcmd", {"st", nullptr}},
