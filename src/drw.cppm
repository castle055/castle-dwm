...
xfont_create(Drw *drw, const char *fontname, FcPattern *fontpattern)
{
	...
	if (fontname) {
		...
		LOG::print {FATAL}("no font specified.");
		std::exit(1);
	}
...
drw_clr_create(Drw *drw, Clr *dest, const char *clrname)
{
	...
	if (!XftColorAllocName(drw->dpy, DefaultVisual(drw->dpy, drw->screen),
	                       ...
	                       clrname, dest)) {
		LOG::print {FATAL}("error, cannot allocate color '{}'", clrname);
		std::exit(1);
	}
}
...
drw_scm_create(Drw *drw, char *clrnames[], size_t clrcount)
{
	...
	if (!drw || !clrnames || clrcount < 2 || !(ret = (XftColor*)std::calloc(clrcount, sizeof(XftColor))))
		return NULL;
...
drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert)
{
	...
	while (1) {
		...
		if (utf8strlen) {
			...
			for (len = std::min(static_cast<unsigned long>(utf8strlen), sizeof(buf) - 1); len && ew > w; len--)
		...
		if (!*text) {
			...
			if (!drw->fonts->pattern) {
				...
				LOG::print {FATAL}("the first font in the cache must be loaded from a font string.");
				std::exit(1);
			}
...
drw_cur_create(Drw *drw, int shape)
{
	...
	if (!drw || !(cur = (Cur*)std::calloc(1, sizeof(Cur))))
		return nullptr;
