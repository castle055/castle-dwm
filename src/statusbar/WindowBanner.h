        ...
        icons::display_external_input {
          {
            .color = state->pressed[1]
                     ? props->theme->accent_bright
                     : state->hover[1]
                       ? props->theme->accent
                       : props->theme->accent_dim,
            .size = size, .bold = true
          }
        }
  ...
  COMPONENT(WindowListItem, {
    ...
    FRAGMENT {
      fragment.append(
        vg::rect { }
        .w($cw()).h($ch())
        .stroke(props->selected ? props->theme->accent : props->theme->accent_dim)
        .stroke_width(1)
      );
    }
    ...
    ON_REDRAW {
      ...
      return {
        ...
        .map_to([&](client_t* client) -> map_to_result_t {
          map_to_result_t c {
            WindowListItem {{props->theme, client, client == props->selected, item_size}}
  ...
  COMPONENT(WindowBanner, {
    ...
    ON_REDRAW {
      ...
      return {
        with {clients.empty()}
        .or_else({
          WindowController {{props->theme, state->monitor ? state->monitor->sel : nullptr}}
          .x(105).h($ch()),
        }),
        WindowList {{props->theme, clients, selected}}
