...
int monitor::bar::draw_statusbar(monitor_t* monitor, int bh, char *stext) {
  int ret, i, w, x, len;
  short isCode = 0;
  char *text;
  char *p;
  ...
  drw_rect(state::drw, x, 0, w, bh, 1, 1);
  x++;
  ...
  while (text[++i]) {
    if (text[i] == '^' && !isCode) {
      ...
      text[i] = '\0';
      w = TEXTW_CSTR(text) - state::lrpad;
      drw_text(state::drw, x, bh / 2, w, 0, 0, text, 0);
  ...
  if (!isCode) {
    w = TEXTW_CSTR(text) - state::lrpad;
    drw_text(state::drw, x, 0, w, bh, 0, text, 0);
  }
...
void monitor::bar::draw_bar(monitor_t* monitor) {
  ...
  drw_rect(state::drw, 0, 0, monitor->ww, state::bh, 1, 1);
  
  /* draw status first so it can be overdrawn by TAGS later */
  //if (m == selmon || 1) { /* status is only drawn on selected monitor */
  tw = monitor->ww - draw_statusbar(monitor, state::bh, (char*)state::stext.c_str());
  ...
  for (i = 0; i < state::config::TAGS.size(); i++) {
    ...
    drw_text(state::drw, x, 3, w - 4, state::bh - 6, state::lrpad / 2 - 2, state::config::TAGS[i].c_str(), urg & 1 << i);
    if (occ & 1 << i)
  ...
  drw_setscheme(state::drw, state::scheme[SchemeNorm]);
  x = drw_text(state::drw, x, 3, w, state::bh - 6, state::lrpad / 2, monitor->ltsymbol, 0);
  
  if (state::key_nav::accepting) {
    x = drw_text(state::drw, x, 3, w, state::bh - 6, state::lrpad / 2, state::key_nav::current_path.c_str(), 0);
  }
  
  if ((w = monitor->ww - tw - x) > state::bh) {
    if (monitor->sel) {
      drw_setscheme(state::drw, state::scheme[monitor == state::selmon ? SchemeSel : SchemeNorm]);
      drw_text(state::drw, x, 0, w, state::bh, (state::lrpad + w - TEXTW(monitor->sel->name)) / 2, monitor->sel->name.c_str(), 0);
      if (monitor->sel->isfloating)
        drw_rect(state::drw, x + boxs + 4, 3, w - 8, state::bh - 6, monitor->sel->isfixed, 0);
    } else {
      drw_setscheme(state::drw, state::scheme[SchemeNorm]);
      drw_rect(state::drw, x, 0, w, state::bh, 1, 1);
    }
  }
  drw_map(state::drw, monitor->barwin, 0, 0, monitor->ww, state::bh);
}
...
void monitor::update_bar_pos(monitor_t* monitor) {
  ...
  if (state::config::showbar) {
    ...
    monitor->wy = state::config::topbar ? monitor->wy + state::bh : monitor->wy;
  } else
    monitor->by = -state::bh;
}
